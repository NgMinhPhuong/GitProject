class bg{
    ...
    draw(){
        ...
        if(this.game.screen.left < 0)
        {
            while(currentLineX + this.game.screen.left < 0)
            currentLineX += GRID_SIZE;
        }
        while( currentLineX <= SCREEN_WIDTH 
            && this.game.screen.left + currentLineX <= GAME_WIDTH){
            this.drawLine(
                {x: currentLineX, y: 0},
        ...
        if(this.game.screen.top < 0)
        {
            while(currentLineY + this.game.screen.top < 0)
            currentLineY += GRID_SIZE;
        }
        while( currentLineY <= SCREEN_HEIGHT 
            && this.game.screen.top + currentLineY <= GAME_HEIGHT){
            this.drawLine(
                {x: 0, y: currentLineY},
